...
export const StudentAddressStep: React.FC<StudentStepComponentProps> = () => {
  ...
  const [createAddress, { loading: isSubmitting }] = useMutation(
    CREATE_STUDENT_ADDRESS,
    {
      ...
      onCompleted: () => {
        // Server-driven step sync in StudentOnboarding
      },
      onError: (error) => {
        setSubmitError(
  ...
  useEffect(() => {
    console.log('[StudentAddressStep] Profile data:', profileData);
    const addresses = profileData?.myStudentProfile?.addresses;
    console.log('[StudentAddressStep] Addresses:', addresses);
    ...
    if (addresses && addresses.length > 0) {
      ...
      console.log('[StudentAddressStep] Home address found:', homeAddress);
