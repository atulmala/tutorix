...
3. **Web GraphQL URL**: If you do **not** set `VITE_GRAPHQL_ENDPOINT`, the client defaults to `http://localhost:3000/api/graphql` ([libs/shared-graphql/src/client/web/endpoint.ts](libs/shared-graphql/src/client/web/endpoint.ts)), which matches local Nx API — so local web + local API usually **just work** without extra env.
